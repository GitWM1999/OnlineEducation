<template>
  ...
  <div class="box5">
    ...
    <el-tab-pane label="课程大纲" name="second">
        <div class="box6" v-for="item in ClassSecondType" :key="item.Class_Id">
           
                <el-button type="text" class="outline" v-if="item.Class_Price==0">{{item.Class_Name}}<span class="free">免费</span></el-button>
                <el-button type="text" class="outline" v-else>{{item.Class_Name}}</el-button>
...
export default ({
    ...
    methods:{
          ...
          this.$axios({
              ...
              this.ClassDetail.forEach(item => {
                  if(item.Class_Id==this.$route.query.id){
                     this.form=item;
                    //  this.playerOptions.sources.src=form.Class_File;
                    this.playerOptions.sources.forEach(item=>{
                        item.src=this.form.Class_File;
          
                    })
                    this.GetClassSecondType(this.form.Class_Second);
                  }
              });
        ...
        GetClassSecondType(id){
            ...
            this.$axios({
                url:"/api/Freeclasses/GetClassSecond?TypeId="+id,
                method:"get"
            }).then(res=>{
               this.ClassSecondType=res.data;
               alert(JSON.stringify(this.ClassSecondType));
            })
        }
    }
})
